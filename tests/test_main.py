...
class EloverblikMcpTests(unittest.TestCase):
    ...
    @patch.dict(
        "os.environ",
        {"METERING_POINT_ID": "000000000000000000", "API_REFRESH_TOKEN": "refresh"},
    )
    @patch("main.get_access_token", return_value="access-token")
    @patch("main.requests.request")
    def test_metering_point_details_uses_default_metering_point(
        self, mock_request, _mock_token
    ):
        ...
        self.assertEqual(
            request_kwargs["json"],
            {"meteringPoints": {"meteringPoint": ["000000000000000000"]}},
        )
        self.assertEqual(
    ...
    def test_fetch_access_token_sends_refresh_token(self, mock_get):
        mock_get.return_value = FakeResponse(json_data={"result": "access-token"})

        result = main.fetch_access_token("refresh-token")
    ...
    @patch("main.get_access_token", return_value="access-token")
    @patch("main.requests.request")
    def test_relation_add_with_web_access_code_encodes_path(
        self, mock_request, _mock_token
    ):
        ...
        self.assertEqual(
            ...
            "https://api.eloverblik.dk/customerapi/api/meteringpoints/meteringpoint/relation/add/000000000000000000/ab%2Fcd%2012",
        )
    ...
    @patch("main.get_access_token", return_value="access-token")
    @patch("main.requests.request")
    def test_timeseries_keeps_existing_end_date_adjustment(
        self, mock_request, _mock_token
    ):
        mock_request.return_value = FakeResponse(json_data={"result": []})
